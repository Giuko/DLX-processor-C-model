...
void draw_left_panel(int step) {
    ...
    MOVE_CURSOR(38, 1);
    printf("Press  [r] restart  [q] quit  [any] next step  [c] continue till the end  ");
}
...
// cpu_load_program
//
// Reads a .mem file generated by the compiler and loads:
//   @TEXT   → IRAM  via cpu_load_instr(handle, word_index, word)
//   @RODATA → RODATA via cpu_load_rodata(handle, absolute_addr, word)
//
// Returns: number of TEXT instructions loaded, or -1 on error.
// ─────────────────────────────────────────────────────────────────────────────
int cpu_load_program(void *handle, FILE *fd) {
    if (!handle || !fd) return -1;
    ...
    while (fgets(line, sizeof(line), fd)) {
        line[strcspn(line, "\r\n")] = 0;
        if (strcmp(line, "@TEXT")   == 0) { sec = SEC_TEXT;   continue; }
        if (strcmp(line, "@RODATA") == 0) { sec = SEC_RODATA; continue; }
        if (line[0] == '\0') continue;

        uint32_t word;
        if (sscanf(line, "%x", &word) != 1) {
            fprintf(stderr, "[LOADER] Invalid line: '%s'\n", line);
            continue;
        }

        if (sec == SEC_TEXT) {
            cpu_load_instr(handle, (uint32_t)text_index, word);
            text_index++;
        } else if (sec == SEC_RODATA) {
            uint32_t addr = RODATA_BASE + (uint32_t)(rodata_index);
            cpu_load_rodata(handle, addr, word);
            rodata_index++;
        }
    }

    printf("[LOADER] TEXT: %d instructions, RODATA: %d words at 0x%08x\n",
           text_index, rodata_index, (unsigned)RODATA_BASE);
    return text_index;
}
